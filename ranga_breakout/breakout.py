...
class Breakout:
    ...
    def place_second_order(self):
        try:
            buy_or_sell = "buy" if self.dir == "sell" else "sell"
            getattr(self, f"_{buy_or_sell}_trade")(self.dct)
            self.dct["fn"] = self.is_buy_or_sell
            self.message = f"{buy_or_sell} order placed"
        except Exception as e:
            fn = self.dct.pop("fn")
            self.message = f"{self.dct['tsym']} encountered {e} while {fn}"
            logging.error(self.message)
